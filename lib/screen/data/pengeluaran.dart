...
class _PengeluaranState extends State<Pengeluaran> {
  ...
  void simpan() async{
    if(tanggalController.text.length < 1 || nominalController.text.length < 1 || keteranganController.text.length < 1){
      ...
      try{
        var insert = await _helper.insert(CashFlowQuery.TABLE_NAME, {"date":"${tanggalController.text.toString()}","nominal":"${nominalController.text.toString()}","keterangan":"${keteranganController.text.toString()}","type":0});
        popScreen(context);
